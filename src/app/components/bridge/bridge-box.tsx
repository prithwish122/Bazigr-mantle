...
const MANTLE_BRIDGE_ADDRESS = "0x118b30B86500239442744A73F1384D97F8C9B63C"; // Mantle bridge address
const SEPOLIA_BRIDGE_ADDRESS = "0xE8aDCF38C12cB70CcEAcE6cb7fbB1e6e5305550B"; // Sepolia bridge address
const MANTLE_TOKEN_ADDRESS = "0x0e4D19ac0ed1CAa536A90C9bb06B6167F1341AFF";
const SEPOLIA_TOKEN_ADDRESS = "0xDf2eEe4b129EA759500c7aDbc748b09cE8487e9c";

export function BridgeBox() {
  const [fromNetwork, setFromNetwork] = React.useState<string>("mantle")
  ...
  React.useEffect(() => {
    if (chainId) {
      if (chainId === 5003) {
        // Currently on Mantle, set as from network
        setFromNetwork("mantle")
  ...
  function handleFromNetworkChange(id: string) {
    setFromNetwork(id)
    if (id === toNetwork) setToNetwork(id === "mantle" ? "sepolia" : "mantle")
  }

  function handleToNetworkChange(id: string) {
    setToNetwork(id)
    if (id === fromNetwork) setFromNetwork(id === "mantle" ? "sepolia" : "mantle")
  }
  ...
  async function handleClaim(reward: number) {
    try {
      ...
      // Switch to Mantle network for claiming
      const targetChainIdHex = "0x138B" // Mantle Sepolia chainId 5003
      try {
      ...
      const contract = new ethers.Contract(MANTLE_TOKEN_ADDRESS as `0x${string}`, bazigrAbi as any, signer)
  ...
  async function onBridge(fromAmount: string) {
    ...
    try {
      ...
      const needsNetworkSwitch =
        (fromNetwork === "mantle" && currentChainId !== 5003) ||
        (fromNetwork === "sepolia" && currentChainId !== 11155111)

      if (needsNetworkSwitch) {
        toast({ title: "Switching Network", description: `Switching to ${fromNetwork === "mantle" ? "Mantle" : "Sepolia"} network...` })
      ...
      if (fromNetwork === "mantle" && toNetwork === "sepolia") {
        // Bridge from Mantle to Sepolia
        await bridgeFromMantleToSepolia(amount)
      } else if (fromNetwork === "sepolia" && toNetwork === "mantle") {
  ...
  async function bridgeFromMantleToSepolia(amount: number) {
    ...
    const tokenContract = new ethers.Contract(MANTLE_TOKEN_ADDRESS, bazigrAbi, signer)
  ...
  async function bridgeFromSepoliaToCelo(amount: number) {
    ...
    const mantleChainId = "0x138B" // Mantle Sepolia chainId 5003
    try {
      await eth.request({ method: "wallet_switchEthereumChain", params: [{ chainId: mantleChainId }] })
    } catch (switchErr: any) {
      if (switchErr?.code === 4902) {
        await eth.request({
          method: "wallet_addEthereumChain",
    ...
    toast({ title: "Step 4/4", description: "Unlocking tokens on Mantle..." })
