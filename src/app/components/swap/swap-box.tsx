...
export function SwapBox() {
  const [fromToken, setFromToken] = React.useState<Token>("MNT")
  ...
  const TOKEN_ADDRESS = "0x0e4D19ac0ed1CAa536A90C9bb06B6167F1341AFF" as `0x${string}`
  const SWAP_ADDRESS = "0x674DEB50b0402bca07c97c1DD34eeD4f9648eE79" as `0x${string}`
  const RATE = 20n
  ...
  function computeTo(from: string): string {
    ...
    if (fromToken === "MNT" && toToken === "BAZ") return (v * 20).toString()
    if (fromToken === "BAZ" && toToken === "MNT") return (v / 20).toString()
    return from
  }
  ...
  async function handleSwapClick() {
    ...
    try {
      ...
      if (fromToken === "MNT" && toToken === "BAZ") {
      ...
      } else if (fromToken === "BAZ" && toToken === "MNT") {
        ...
        toast({ title: "Unsupported pair", description: "Choose MNT ↔ BAZ" })
        return
      }
