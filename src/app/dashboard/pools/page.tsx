...
export default function DeFiPage() {
  ...
  // Swap state
  const [swapFromToken, setSwapFromToken] = useState<"BAZ" | "WMANTLE">("BAZ");
    ...
    const fetchData = async () => {
      try {
        ...
        const [baz, wcelo, celo, lp] = await Promise.all([
          ...
          publicClient.readContract({
            address: DEFI_CONTRACTS.WMANTLE as Address,
            abi: ERC20_ABI,
            functionName: "balanceOf",
            args: [address],
            blockNumber: latestBlock,
          ...
          publicClient.readContract({
            address: DEFI_CONTRACTS.BAZ_WMANTLE_PAIR as Address,
            abi: ERC20_ABI,
            functionName: "balanceOf",
            args: [address],
            blockNumber: latestBlock,
    ...
    const calculateOutput = async () => {
      try {
        ...
        const path = swapFromToken === "BAZ"
          ? [DEFI_CONTRACTS.BAZ_TOKEN, DEFI_CONTRACTS.WMANTLE]
          : [DEFI_CONTRACTS.WMANTLE, DEFI_CONTRACTS.BAZ_TOKEN];
  ...
  const handleAddLiquidity = async () => {
    ...
    if (!bazAmount || !wceloAmount) {
      toast({
        title: "Error",
        description: "Please enter both BAZ and WMANTLE amounts",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (wceloBal < wceloAmt) {
        toast({
          title: "Insufficient Balance",
          description: `You need ${formatUnits(wceloAmt, 18)} WMANTLE but only have ${formatUnits(wceloBal as bigint, 18)}`,
          variant: "destructive",
        });
        setIsLoading(false);
        return;
      ...
      const bazTransferHash = await walletClient.writeContract({
        ...
        args: [DEFI_CONTRACTS.BAZ_WMANTLE_PAIR as Address, bazAmt],
      });
      ...
      toast({
        title: "Step 2/3: Transferring WMANTLE...",
        description: `Sending ${formatUnits(wceloAmt, 18)} WMANTLE to pool`,
      });
      ...
      const wceloTransferHash = await walletClient.writeContract({
        ...
        abi: ERC20_ABI,
        functionName: "transfer",
        args: [DEFI_CONTRACTS.BAZ_WMANTLE_PAIR as Address, wceloAmt],
      });
      ...
      if (wceloReceipt.status !== 'success') {
        throw new Error("WMANTLE transfer failed");
      }
    ...
    try {
      ...
      const hash = await walletClient.writeContract({
        ...
        args: [
          ...
          DEFI_CONTRACTS.WMANTLE as Address,
          lpAmt,
          BigInt(0),
          BigInt(0),
          address,
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      const tokenIn = swapFromToken === "BAZ" ? DEFI_CONTRACTS.BAZ_TOKEN : DEFI_CONTRACTS.WMANTLE;
      const path = swapFromToken === "BAZ"
        ? [DEFI_CONTRACTS.BAZ_TOKEN, DEFI_CONTRACTS.WMANTLE]
        : [DEFI_CONTRACTS.WMANTLE, DEFI_CONTRACTS.BAZ_TOKEN];
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <Card className="backdrop-blur-xl bg-white/5 border border-white/10">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-white/80">WMANTLE Amount</label>
                    <Input
                      type="number"
                      placeholder="0.0"
                      value={wceloAmount}
                    ...
                    />
                    <p className="text-xs text-white/50">
                      Balance: {parseFloat(wceloBalance).toFixed(4)} WMANTLE
                    </p>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/10">
                    ...
                    <p className="text-sm text-white/80">WMANTLE: {parseFloat(poolReserves.wcelo).toFixed(2)}</p>
                  </div>
                </CardContent>
              </Card>
